...
nUniqueDuos <- length(sort(table(restrictedDuos$restrictedDuo), decreasing = T))
...
pdf(file = 'plots/dotchart_usageTOP.pdf',
    height = 11,
    width = 8.5,
    pointsize = 12)
{
dotchart(as.numeric(rev(t1)),
  ...
  points(as.numeric(rev(t1)), pch = 20, 1:nUniqueDuos)
...
pdf(file = 'plots/dotchart_placementTOP.pdf',
    height = 11,
    width = 8.5,
    pointsize = 12)
{
...
grid(nx = NULL, ny = nUniqueDuos+1)

points(temp1$avgPlacement,nUniqueDuos:1, col = '#00bf7d', pch = 20)
points(temp1$medPlacement,nUniqueDuos:1, col = '#5928ed', pch = 20)
points(temp1$highestPlacement,nUniqueDuos:1, pch = 20)
